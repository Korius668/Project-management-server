...
def test_create_project(client, mock_service):
    ...
    response = client.post(
        "/projects/", json={"name": "Test Project", "description": "Opis"}
    )
...
def test_invite_user_to_project(client, mock_service):
    ...
    response = client.post(
        ...
        params={"target_id": str(target_id), "role": "viewer"},
    )
    assert response.status_code == 201
...
def test_update_user_role(client, mock_service):
    ...
    response = client.put(
        f"/projects/{project_id}/members/{target_id}/role", params={"role": "owner"}
    )
    assert response.status_code == 200
