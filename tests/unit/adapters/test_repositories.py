...
class TestSqlAlchemyProjectsRepository:
    ...
    def test_update_project_not_found(self, sample_project):
        ...
        result = None
        with pytest.raises(
            ProjectNotFoundError, match=f"Project with id {project.id} not found"
        ):
    ...
    def test_delete_project_not_exists(self):
        ...
        # When
        with pytest.raises(
            ProjectNotFoundError, match=f"Project with id {project_id} not found"
        ):
            repo.delete(project_id)
...
class TestSqlAlchemyDocumentsRepository:
    ...
    def test_get_document_not_found(self):
        ...
        # When
        with pytest.raises(
            DocumentNotFoundError, match=f"Document with id {document_id} not found"
        ):
            result = repo.get(document_id)
    ...
    def test_update_document_not_found(self, sample_document):
        ...
        with pytest.raises(
            DocumentNotFoundError,
            match=f"Document with id {sample_document.id} not found",
        ):
