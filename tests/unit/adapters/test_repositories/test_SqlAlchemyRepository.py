...
class TestSqlAlchemyRepository:
    ...
    def sample_membership(self, sample_project_id, sample_user_id):
        return ProjectMembership(
            project_id=sample_project_id, user_id=sample_user_id, role=ProjectRole.owner
        )
    ...
    @patch(
        "app.adapters.repositories.sqlalchemy.head_repository.SqlAlchemyUsersRepository"
    )
    ...
    def test_get_user_success(
        self, mock_users_repo_class, repository, mock_session, sample_user
    ):
        # Arrange
        mock_users_repo = Mock()
        mock_users_repo_class.return_value = mock_users_repo
    ...
    @patch(
        "app.adapters.repositories.sqlalchemy.head_repository.LocalFileStorageAdapter"
    )
    @patch(
        "app.adapters.repositories.sqlalchemy.head_repository.SqlAlchemyDocumentsRepository"
    )
    @patch(
        "app.adapters.repositories.sqlalchemy.head_repository.SqlAlchemyProjectMembershipsRepository"
    )
    @pytest.mark.asyncio
    async def test_upload_document_success(
        self,
        mock_memberships_repo_class,
        mock_docs_repo_class,
        mock_storage_class,
        repository,
        mock_session,
        sample_project_id,
        ...
        # Assert
        mock_memberships_repo.get.assert_called_once_with(
    ...
    @pytest.mark.asyncio
    async def test_upload_document_insufficient_permissions(
        self,
        mock_memberships_repo_class,
        repository,
        mock_session,
        ...
        with pytest.raises(
            InsufficientPermissionsError,
            match="Only editors and owners can upload documents",
        ):
            await repository.upload_document(
                sample_project_id, sample_user_id, mock_upload_file
            )
    ...
    async def test_download_document_success(
        ...
        # Act
        file, document = await repository.download_document(
        ...
        mock_memberships_repo.get.assert_called_once_with(
            sample_document.project_id, sample_user_id
        )
    ...
    async def test_download_document_not_found(
        ...
        with pytest.raises(
            DocumentNotFoundError,
            match=f"Document with id {sample_document_id} not found",
        ):
    ...
    @patch(
        "app.adapters.repositories.sqlalchemy.head_repository.SqlAlchemyProjectsRepository"
    )
    @patch(
        "app.adapters.repositories.sqlalchemy.head_repository.SqlAlchemyProjectMembershipsRepository"
    )
    def test_create_project_success(
        ...
        result = repository.create_project(
            "Test Project", "Test Description", sample_user_id
        )
    ...
    def test_get_project_not_found(
        ...
        # Act & Assert
        with pytest.raises(
            ProjectNotFoundError, match=f"Project with id {sample_project_id} not found"
        ):
    ...
    def test_invite_user_to_project_success(
        ...
        sample_project = Project(
            id=sample_project_id, name="Test", description="Test", owner_id=inviter_id
        )
        inviter_membership = ProjectMembership(
            project_id=sample_project_id, user_id=inviter_id, role=ProjectRole.owner
        )
        invited_user = User(
            id=invited_user_id,
            name="invited",
        ...
        mock_projects_repo.get.return_value = sample_project
        mock_memberships_repo.get.side_effect = [
            inviter_membership,
            None,
        ]  # inviter exists, invited doesn't
        ...
        # Act
        result = repository.invite_user_to_project(
            sample_project_id, inviter_id, invited_user_id, ProjectRole.editor
        )
    ...
    def test_invite_user_already_member(
        ...
        existing_membership = ProjectMembership(
            project_id=sample_project_id,
            user_id=invited_user_id,
            role=ProjectRole.viewer,
        )
        ...
        # Act & Assert
        with pytest.raises(
            UserAlreadyMemberError, match="User is already a member of this project"
        ):
            repository.invite_user_to_project(
                sample_project_id, inviter_id, invited_user_id, ProjectRole.editor
            )
    ...
    @pytest.mark.asyncio
    async def test_upload_document_no_membership(
        self,
        mock_memberships_repo_class,
        repository,
        mock_session,
    ...
    def test_get_user_projects_success(
        ...
        memberships = [
            ProjectMembership(
                project_id=project1_id, user_id=sample_user_id, role=ProjectRole.owner
            ),
            ProjectMembership(
                project_id=project2_id, user_id=sample_user_id, role=ProjectRole.editor
            ),
        ]
        ...
        projects = [
            ...
            Project(
                id=project2_id, name="Project 2", description="Desc 2", owner_id=uuid4()
            ),
        ]
    ...
    def test_update_project_insufficient_permissions(
        ...
        with pytest.raises(
            InsufficientPermissionsError,
            match="You don't have permission to edit this project",
        ):
    ...
    def test_delete_project_success(
        ...
        mock_memberships_repo.delete_by_project.assert_called_once_with(
            sample_project_id
        )
    ...
    def test_delete_project_not_owner(
        ...
        with pytest.raises(
            InsufficientPermissionsError,
            match="Only project owner can delete the project",
        ):
    ...
    def test_update_user_role_success(
        ...
        updater_membership = ProjectMembership(
            project_id=sample_project_id, user_id=updater_id, role=ProjectRole.owner
        )
        ...
        result = repository.update_user_role(
            sample_project_id, updater_id, target_user_id, ProjectRole.editor
        )
    ...
    def test_update_user_role_cannot_change_owner(
        ...
        target_membership = ProjectMembership(
            project_id=sample_project_id, user_id=target_user_id, role=ProjectRole.owner
        )
        ...
        # Act & Assert
        with pytest.raises(
            InsufficientPermissionsError, match="Cannot change owner role"
        ):
    ...
    def test_remove_user_from_project_success(
        ...
        remover_membership = ProjectMembership(
            project_id=sample_project_id, user_id=remover_id, role=ProjectRole.owner
        )
        ...
        # Act
        result = repository.remove_user_from_project(
            sample_project_id, remover_id, target_user_id
        )

        # Assert
        mock_projects_repo.get.assert_called_once_with(sample_project_id)
        assert mock_memberships_repo.get.call_count == 2
        mock_memberships_repo.delete.assert_called_once_with(
            sample_project_id, target_user_id
        )
    ...
    def test_remove_user_cannot_remove_owner(
        ...
        # Act & Assert
        with pytest.raises(
            InsufficientPermissionsError, match="Cannot remove project owner"
        ):
            repository.remove_user_from_project(
                sample_project_id, remover_id, target_user_id
            )
    ...
    def test_get_project_info_success(
        ...
        member_user = User(
            id=uuid4(), name="member", email="member@test.com", password_hash="hash"
        )
    ...
    async def test_upload_project_documents_insufficient_permissions(
        self,
        mock_memberships_repo_class,
        repository,
        mock_session,
        ...
        with pytest.raises(
            ...
            await repository.upload_project_documents(
                sample_project_id, sample_user_id, [mock_upload_file]
            )
