...
class TestProjectsAPI:
    ...
    def test_create_project_success(self, test_client, authenticated_user, sample_project_data):
    ...
    def test_get_project_info_success(self, test_client, authenticated_user, test_project):
    ...
    def test_update_project_success(self, test_client, authenticated_user, test_project):
    ...
    def test_update_project_partial_success(self, test_client, authenticated_user, test_project):
    ...
    def test_delete_project_success(self, test_client, authenticated_user, test_project):
...
class TestProjectMembership:
    ...
    def test_invite_user_to_project_success(self, test_client, authenticated_user, test_project, second_user):
        """Test inviting user to project via API."""
        response = test_client.post(
            f"/projects/{test_project['id']}/invite",
            params={
                "target_id": second_user["user"]["id"],
                "role": "editor"
            },
            headers=authenticated_user["headers"]
        )
    ...
    def test_invite_user_without_permission_fails(self, test_client, second_user, test_project):
    ...
    def test_update_user_role_success(self, test_client, authenticated_user, test_project, second_user):
...
class TestProjectDocuments:
    ...
    def test_upload_documents_without_permission_fails(self, test_client, test_project, sample_file_content):
...
class TestProjectPermissions:
    ...
    def test_owner_can_access_all_operations(self, test_client, authenticated_user, test_project):
    ...
    def test_non_member_cannot_access_project(self, test_client, test_project):
        ...
        login_response = test_client.post(
            "/auth/login",
            params={"username": other_user_data["username"], "password": other_user_data["password"]}
        )
        assert login_response.status_code == 200
