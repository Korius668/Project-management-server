...
class TestDocumentsAPI:
    ...
    def test_upload_document_success(
        self, test_client, authenticated_user, test_project, sample_file_content
    ):
        ...
        response = test_client.post(
            "/documents/", files=files, data=data, headers=authenticated_user["headers"]
        )
    ...
    def test_upload_document_without_auth_fails(
    ...
    def test_upload_document_missing_file_fails(
        ...
        response = test_client.post(
            "/documents/", data=data, headers=authenticated_user["headers"]
        )

        assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY

    def test_upload_document_missing_project_id_fails(
        self, test_client, authenticated_user, sample_file_content
    ):
        """Test uploading document without project ID fails."""
        files = {"file": ("test.txt", sample_file_content, "text/plain")}

        response = test_client.post(
            "/documents/", files=files, headers=authenticated_user["headers"]
        )
    ...
    def test_download_document_success(
        ...
        download_response = test_client.get(
            f"/documents/{doc_data['id']}", headers=authenticated_user["headers"]
        )

        assert download_response.status_code == status.HTTP_200_OK
        assert download_response.content == sample_file_content

    def test_download_document_without_auth_fails(
        self, test_client, authenticated_user, test_project, sample_file_content
    ):
    ...
    def test_download_nonexistent_document_fails(self, test_client, authenticated_user):
        ...
        response = test_client.get(
            f"/documents/{fake_id}", headers=authenticated_user["headers"]
        )
    ...
    def test_update_document_partial_success(
        self, test_client, authenticated_user, test_project, sample_file_content
    ):
    ...
    def test_delete_document_without_auth_fails(
        self, test_client, authenticated_user, test_project, sample_file_content
    ):
